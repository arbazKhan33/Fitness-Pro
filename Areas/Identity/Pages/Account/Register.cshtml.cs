...
namespace FitnessPro.Areas.Identity.Pages.Account
{
    ...
    public class RegisterModel : PageModel
    {
        ...
        private FitnessUser CreateUser()
        {
            try
            {

                FitnessUser user = Activator.CreateInstance<FitnessUser>();
            ...
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(FitnessUser)}'. " +
                    $"Ensure that '{nameof(FitnessUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<FitnessUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<FitnessUser>)_userStore;
        }
    }
}
